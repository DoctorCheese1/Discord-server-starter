...
function getFileEditability(fullPath) {
  ...
  if (!TEXT_EXTENSIONS.has(ext)) {
    return {
      editable: false,
      reason: `${path.basename(fullPath)} cannot be edited (file type not allowed)`
    };
  }
  ...
  if (size > MAX_FILE_BYTES) {
    return {
      editable: false,
      reason: `${path.basename(fullPath)} cannot be edited (file too large)`
    };
  }
...
function listEditableFiles(cwd, maxDepth = 16) {
  const results = [];
  const emptyFolders = [];
    ...
    try {
      ...
      if (prefix) {
        results.push({
          path: prefix,
          editable: false,
          reason: `${path.basename(prefix)} cannot be edited (folder cannot be read)`
        });
      }
      return true;
    }
    ...
    for (const entry of entries) {
      ...
      if (!entry.isFile()) {
        results.push({
          path: rel,
          editable: false,
          reason: `${entry.name} cannot be edited (unsupported entry type)`
        });
        subtreeHasVisibleEntries = true;
        continue;
      }

      let editability = { editable: false, reason: `${entry.name} cannot be edited (metadata read failed)` };
      try {
        editability = getFileEditability(full);
      } catch {}
...
function listFiles(cwd, maxDepth = 16) {
  return listEditableFiles(cwd, maxDepth);
}
